...
static float get_pixel(image* img, unsigned int x, unsigned int y, unsigned int c){
    // x = image width, y= image height, c = channel 
    assert(x<(img->w)&&y<(img->h)&&c<(img->c));
    return img->data[c*img->w*img->h+y*img->w+x];
}
static float set_pixel(image *img, float value, unsigned int x, unsigned int y, unsigned int c){
...
void horizontal_flip(image *img){
     ...
     * */
    float *tmp = (float *)malloc(img->w*img->h*img->c*sizeof(float));
    for(short z=0;z<img->c;z++){
        for(short y=0;y<img->h;y++){
            register int st_idx = y * img->w + z*img->w*img->h;
            for(short x=0;x<img->w;x++)
                tmp[st_idx+x] = img->data[st_idx+img->w-x];
        }
    }
}
...
{
    image resized, part;
    make_image(&resized, w, h, img->c);
    make_image(&part,  w, img->h, img->c);
    float   w_scale = (img->w-1) * 1.0 / (w-1),
            h_scale = (img->h-1) * 1.0 / (h-1);
    register float val;
    ...
    memcpy(img, &resized, sizeof(image));
    free_image(&part);
    free_image(&resized);
}
...
void get_next_batch(int n, float *X, int *Y, int w, int h, int c, int CLASSES, FILE *fp){
    /**
